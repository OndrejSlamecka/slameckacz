...
Another approach is to consider the expected value of a random variable $X$. If the expected value of $X$ is μ
then you're guaranteed there exists an object for which $X$ is at least μ and an object for which $X$ is at most $μ$
(possibly the same object). Stated formally as a lemma that follows from a short argument based on contradiction:
